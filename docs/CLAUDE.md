...
App pessoal para controle de banco de horas de estágio. O usuário bate ponto de entrada num horário de referência configurável (padrão **15:00**, ver "Horário do ponto configurável" abaixo) e usa o app pra registrar o horário de **saída**, calculando automaticamente quanto tempo extra foi feito (ou descontado, se saiu antes da hora). Foco 100% mobile: usado no celular, no fim do expediente, pra tirar uma foto do comprovante e já sair registrado.

App está organizado sem framework, com HTML + CSS + JS puro. Stack: sem build/bundler, sem dependências de runtime além de Firebase (Auth + Firestore) e JSZip (via CDN), hospedado no GitHub Pages. A entrada principal continua sendo o `index.html` na raiz, com estilos compartilhados em `assets/css/style.css`, módulo compartilhado em `assets/js/shared.js` (`Store`, `calcularExtra`, `isDiaUtil`, `initShell`, geocoding, config de período etc.), scripts próprios de cada tela (`assets/js/script.js` pra home, `assets/js/calendario.js` e `assets/js/banco-horas.js`) e telas-fonte separadas em `views/` (`calendario.html`, `banco-horas.html`, `splash.html`). No deploy do GitHub Pages, o workflow gera aliases de rota `calendario/index.html`, `banco-horas/index.html` e `splash/index.html`, permitindo URLs sem `.html`; esses arquivos são artefatos gerados e não devem ser criados manualmente no repositório. `views/base.html` existe no repo como template `{{content}}`/`{{root}}` mas não é referenciado por nenhuma tela atual — código morto/experimento, não usar como referência de estrutura.
...
### Horário do ponto configurável

O horário de referência ("ponto") não é mais fixo em 15:00 no código — é configurável pelo usuário (`App.getPointTimeConfig`/`setPointTimeConfig`, `shared.js`, salvo em `localStorage` na chave `bancoHoras_horarioPonto`; padrão `{hour:15, minute:0}`). Editável no Calendário, no mesmo `periodModal` do botão de engrenagem (`periodConfigMode`, campo `point`), com selects de hora/minuto (`periodPointHourSelect`/`periodPointMinuteSelect` em `views/calendario.html`). Documentação abaixo usa 15:00 só como exemplo padrão — a regra de cálculo é sempre relativa ao horário configurado, nunca hardcoded.

**Horário histórico por registro**: cada registro salvo grava o campo `ponto` com o horário de ponto vigente **no momento do registro** (`reg.ponto`, gravado em `script.js`/`calendario.js` ao salvar). Detalhe do dia, relatório em imagem e qualquer exibição de um registro já salvo usam `reg.ponto` (com fallback pro config atual só se o registro for antigo e não tiver o campo) — nunca o valor ao vivo de `getPointTimeLabel()` para dado histórico, senão mudar a config depois quebraria a consistência de registros antigos.

### Cálculo do tempo extra

Ponto de referência: horário configurado acima (padrão **15:00**). Tolerância fixa: **10 minutos** depois do ponto, sempre — `TOLERANCIA_MINUTOS = 10` (`shared.js`), somado ao horário do ponto configurado (não é mais um valor absoluto tipo "15:10" fixo).

- Saída **antes do ponto** → desconta. `extra = saída − ponto` (negativo).
- Saída entre **ponto e ponto+10min** (tolerância) → **neutro**, não conta nada (`extra = 0`).
- Saída **depois de ponto+10min** → soma tempo extra, descontando a tolerância. `extra = saída − (ponto + 10min)`.
...
> **Limitação conhecida**: o cálculo (`calcularExtra`, `shared.js`) soma minutos linearmente sem `% 1440`, então se o ponto for configurado muito perto da meia-noite (ex.: 23:55+), a janela de tolerância pode ultrapassar o fim do dia e a saída nunca vai bater como "extra" mesmo que o rótulo de tolerância (`getPointTimeToleranceLabel`, que arredonda com `%24`) mostre um horário do dia seguinte como se fosse alcançável. Configurações de ponto perto da meia-noite não são um caso de uso real do app (horário de estágio), então não há tratamento especial — só documentado aqui pra não surpreender.
...
- **Fallback local quando autenticado, mas Firestore falha** → `Store._localKey(base)` (`shared.js`) sufixa a chave com `_{uid}` (`bancoHoras_registros_{uid}`, `bancoHoras_baixas_{uid}`, `bancoHoras_relatorios_{uid}`) sempre que existe usuário logado, isolando o cache de fallback por conta — evita que dois usuários diferentes no mesmo navegador/dispositivo compartilhado leiam o cache local um do outro se o Firestore cair. Sem login, a chave continua sem sufixo (`bancoHoras_registros` etc.), igual ao comportamento original.
- Migração automática `localStorage → Firestore` no primeiro login: lê especificamente as chaves **sem sufixo** (dado deixado por uso anônimo antes do login), nunca as chaves já namespaced por uid.
- **Limite de usuários Google** → as Cloud Functions `limitarUsuariosGoogle` e `removerUsuarioGoogleDoLimite` (`functions/index.js`) usam `beforeUserCreated`/`beforeUserDeleted`, a contagem real do Firebase Authentication e uma transação em `config/limiteUsuarios` para permitir no máximo 3 contas Google ativas. O quarto cadastro é rejeitado antes da criação da conta; a trava não deve ser substituída por validação somente no frontend. As funções precisam estar publicadas e o blocking de Authentication habilitado.
...
O objeto `Store` abstrai isso: `getAll()`, `get(key)`, `set(key, data)`, `getAllBaixas()`, `addBaixa(key, data)`, `getAllRelatorios()`, `setRelatorio(key, data)` — todos **async** (retornam Promise).
...
users/{uid}/
  registros/{YYYY-MM-DD}/
    saida: "HH:MM"
    extraMin: number
    ponto: "HH:MM"           // horário do ponto vigente NO MOMENTO do registro (ver "Horário do ponto configurável")
  ...
  relatorios/{YYYY-MM-01}/   // chave = App.toKey(anchor) do período, ex. "2026-08-01" pro período 26/08–25/09
    periodo: "26/08/2026 – 25/09/2026"
    geradoEm: "2026-09-25T18:03:00.000Z"   // ISO string, momento da geração
    imagem: "data:image/png...base64"       // mesma imagem gerada pelo botão "Exportar relatório"
    nomeArquivo: "banco-horas_26-08_a_25-09-2026.png"
```
...
### Histórico de relatórios (auto-save por período)

- Toda vez que um registro é salvo (câmera na home ou manual no Calendário) e a data desse registro é o **último dia útil do período de pagamento** ao qual ela pertence, o app gera e salva sozinho o relatório em imagem daquele período inteiro — sem ação nenhuma do usuário. Ex.: período configurado 26→25, ao registrar a saída do dia 25 (ou do último dia útil antes disso, se o 25 cair num fim de semana/feriado), o relatório desse ciclo já fica disponível pra baixar.
- Detecção do "último dia útil do período": `App.isUltimoDiaUtilDoPeriodo(date)` (`shared.js`) — verifica se `date` é dia útil e se não existe nenhum outro dia útil entre `date` e o fim do período (`getPeriodBounds`). Cobre tanto o caso do período terminar num dia útil normal quanto o período terminar num fim de semana/feriado (nesse caso o gatilho é o último dia útil anterior).
- Geração e persistência: `App.gerarESalvarRelatorioPeriodo(anchor)` (`shared.js`) chama `App.gerarImagemRelatorioPeriodo(anchor)` — a mesma função usada pelo botão manual "Exportar relatório" do Calendário (`calendario.js`, `#exportarBtn`) — e grava o resultado em `Store.setRelatorio(periodoKey, {...})`, na subcoleção `relatorios` (ver "Estrutura no Firestore" acima). Cada geração **sobrescreve** o relatório existente daquele período (mesma chave), então se depois o usuário registrar manualmente um dia esquecido do mesmo período, o relatório salvo não se atualiza sozinho — só regenera se o último dia útil for salvo de novo.
- Exibido no Banco de Horas, aba "Registros", no card **"Histórico de relatórios"** (acima do "Histórico de registros"): lista todos os períodos já gerados (mais recente primeiro), cada linha com o rótulo do período, a data de geração e um botão de download direto da imagem (`renderHistoricoRelatorios`, `banco-horas.js`).
- Limpo junto com registros/baixas do ano anterior na limpeza automática destrutiva (`Store.removeRelatoriosMany`, ver seção acima) — mesma regra, mesmo risco.

### Comprovante de registro diário (câmera e manual)

- `App.gerarComprovanteRegistro({foto, data, horario, localizacao, usuario, saldo})` (`shared.js`) gera um recibo em Canvas: logo, título "Comprovante de Registro", a foto tirada/anexada centralizada, e uma tabela única (mesmo card/borda) onde a primeira linha é uma faixa cor do header (`#5F674F`, cantos arredondados seguindo o card) com **Data** à esquerda e **Usuário** à direita (texto branco, sem rótulo), seguida por: **Horário do ponto** (horário configurado no momento, ver "Horário do ponto configurável") / **Horário de saída** / **Saldo** (3 colunas na mesma linha — Saldo mostra só o resultado com sinal, `+` acima da tolerância, `-` desconto, sem sinal no neutro, via `App.formatarSaldoTexto(extraMin)`, com a nota em **negrito preto** "Tolerância até HH:MM" — `App.getPointTimeToleranceLabel()` — em fonte menor logo abaixo do valor); e por último **Localização** em linha única full-width. Mesmo estilo visual dos outros geradores (relatório do período em `shared.js`/`calendario.js` e recibo de baixa em `banco-horas.js`). O resultado (`data:image/png`) é o que vira o campo `comprovante` salvo — a foto crua em si não é persistida separadamente.
...
- Registro manual não captura GPS ao vivo (mesma regra da Baixa); ele usa a localização padrão configurada em "Configurações Gerais" > "Localização padrão" (`App.getDefaultLocationConfig()`, `localStorage` key `bancoHoras_localizacaoPadrao`), ou fica sem localização se nenhuma tiver sido definida. O recibo já sai completo na primeira geração, sem necessidade de regenerar depois.
...
1. **Splash screen** — ao abrir a raiz do app (URL sem `index.html`), o app mostra `views/splash.html` por 2 segundos com logo e nome da empresa, e depois redireciona de volta para a home na mesma sessão. Acesso direto à URL com `index.html` é normalizado para a raiz.
...
4. **Calendário** — navegação por **período configurável** (padrão 26→25, editável via botão de engrenagem `periodSettingsBtn` que abre o `periodModal`, salvo em `localStorage`; o mesmo modal também tem o modo pra configurar o **horário do ponto**, ver "Horário do ponto configurável"), indicadores visuais por dia: hoje (classe `today`), preenchido (`filled` + `filled-negative/neutral/positive`), pendente/sem registro (classe `pending`, borda vermelha — **só aplica pra dias passados, `key < hojeKey`; hoje sem registro nunca fica marcado como pendente, só como "hoje"**), fim de semana/feriado (`off`, cinza, não clicável). Clicar num dia útil sempre abre o detalhe/formulário de registro manual (com modal de seleção de horário); o card mostra o rótulo "Período", o botão de engrenagem pra alterar início/fim do período e horário do ponto, e o botão **"Exportar relatório"** (`#exportarBtn`, `gerarImagem()`/`App.gerarImagemRelatorioPeriodo`) que baixa a imagem do período em navegação — a mesma função usada pelo auto-save do Histórico de relatórios no Banco de Horas. Navegação de período (‹ ›) segue os 12 períodos do ano civil com registro (mesmo modelo do Banco de Horas, ver seção "Período de pagamento"); dias antes do período do seu primeiro login continuam bloqueados pra registro/detalhe (`getPrimeiroPeriodoPermitido`), independente de até onde a navegação alcança.
5. **Detalhe do dia / Registro manual** — dentro do calendário, ao clicar num dia sem registro abre o formulário de registro manual (`renderFormManual`, wrapper `.form-manual`): campo "Horário Saída" via botão que abre modal de seleção de horário (hora/minuto), campo "Comprovante" com seleção de imagem da galeria, botão "Salvar registro". Dia já registrado mostra o detalhe: horário de ponto (o valor gravado naquele registro, `reg.ponto` — ver "Horário do ponto configurável"), horário de saída, localização (link pro Google Maps + endereço resolvido por **geocodificação reversa** via Nominatim/OSM, cacheado em memória, com fallback pra lat/lng cru se a API falhar), tempo extra do dia, comprovante (miniatura + visualizar + baixar).
6. **Banco de Horas** — saldo total do período atual, lista paginada de registros (`REG_POR_PAGINA = 6`), rótulo "Período" acima do intervalo. Aba "Registros": card **"Histórico de relatórios"** (relatórios de período salvos automaticamente, ver seção própria) logo acima do "Histórico de registros", botão "Baixar comprovantes do período" que gera um ZIP (via JSZip) com os comprovantes do período selecionado. Aba "Baixas": histórico de baixas + botão "Dar baixa no saldo".
...
- **Rotas limpas**: o workflow gera `calendario/index.html`, `banco-horas/index.html` e `splash/index.html` no artefato, publicados como `/calendario/`, `/banco-horas/` e `/splash/`; esses diretórios não são fontes do repositório.
