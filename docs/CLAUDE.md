...
- **Tipografia**: sans-serif do sistema pra UI (labels, botões, inputs); serifada (Georgia) só nos títulos de saudação/cabeçalho de tela ("Bom dia, ...", "Calendário", "Banco de Horas", "Período"). Nas telas internas, os títulos principais "Calendário" e "Banco de Horas" usam tamanho levemente ampliado, com espaçamento entre letras mais fechado, e a linha da data logo abaixo também aparece maior que no header da home — dá o tom "gerado por IA/personalizado" que o projeto buscou.
...
- Sem framework, sem bundler: mudanças de estrutura visual podem ficar distribuídas entre `index.html`, `views/*.html`, `assets/css/style.css` e `assets/js/script.js`.
